...
JOY_NAME_TO_ID_MAPPING = { name:id for id, name in JOY_AXIS_To_NAME_MAPPING.items()}

class JoystickAxisControl:

    def __init__(self, name=str, **kwargs) -> None:
        if name not in JOY_NAME_TO_ID_MAPPING:
            raise Exception(f"Invalid joy axis name, it should be one of { ''.join(list(JOY_AXIS_To_NAME_MAPPING.values())) }")
        self.name = name
    ...
    def set_value(self, raw_value):
        ...
        if self.transform_to_float:
            self.transformed_val = (((raw_value + 1) * self.new_range)/ 2.0) + self.min_val #old range is 1 - (-1) = 2
        elif self.transform_to_int:
            self.transformed_val = int((((raw_value + 1) * self.new_range)/ 2.0) + self.min_val) #old range is 1 - (-1) = 2
...
class JoystickAccumulator(JoystickAxisControl):
    ...
    def update_acc_value(self, change):
        ...
        self.accumulated_val = int(min(max(temp, self.min_acc_val), self.max_acc_val))
...
class XboxController:
    ...
    def __init__(self) -> None:
        ...
        self.current_button_states = { name:False for name in BUTTON_ID_NAME_MAPPING.values() }
