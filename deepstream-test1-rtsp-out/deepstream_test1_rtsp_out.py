...
import sys
import argparse
DEEPSTREAM_DIR = r'/opt/nvidia/deepstream/deepstream-6.0/sources/deepstream_python_apps/apps'
# sys.path.append('../')
sys.path.append(DEEPSTREAM_DIR)
import gi
...
def main(args):
    ...
    factory.set_launch(
        "( udpsrc name=pay0 port=%d buffer-size=524288 caps=\"application/x-rtp, media=video, clock-rate=90000, encoding-name=(string)%s, payload=96 \" )" % (updsink_port_num, codec))
